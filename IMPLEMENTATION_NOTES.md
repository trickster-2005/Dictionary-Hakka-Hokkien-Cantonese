...
- union-find 同義詞分組的上限(`MAX_COMPONENT_SIZE = 80`，在 `build_db.py`)是
  人工抽查資料分佈後訂出的經驗值，不是精確算出來的最佳解。之後如果任一來源資料
  大幅更新，這個數字可能需要重新校準(重新跑一次分組大小分佈統計、人工抽查中大型
  分組的內容)。
...
### `build_db.py` 整合邏輯
1. 依序呼叫三個 parser，合併成 `all_rows`。
2. 對每筆 row 的 `strong_aliases`(=各語言的 synonym 來源欄位)跑 `UnionFind`，
   算出跨詞條、甚至跨語言的同義詞分組(見 1.8)。
3. 分組成員數 ≤80 才展開成 `kind='synonym'` 別名，>80 的分組退回只用該筆 row 自己
   的直接別名(`kind='gloss'`)。
...
### 資料庫規模(目前部署在 gh-pages 上的版本，對應 main commit `615bc8d`)
- 華語詞條(`zh_terms`)共 46,008 筆
- `entries` 共 76,737 筆，拆解如下:
  - 粵語(yue):31,147 筆
  - 台語(nan):14,998 筆
  - 客語(hak，海陸+四縣合計):30,592 筆
- `aliases`(含 union-find 展開後)共 197,094 筆，同義詞分組共 35,534 組
  (其中 4 組因超過 `MAX_COMPONENT_SIZE=80` 被退回只用直接別名)
- `dictionary.sqlite` 檔案大小約 54MB(超過 GitHub 建議的單檔 50MB，尚未到 100MB
  硬限制)
...
- 跨語言「同義詞網絡」視覺化：union-find 分組本身已經是一個語意關聯圖，可以做成
  「這個詞在三語言裡有哪些相關字詞」的圖表檢視，而不只是拿來排序。
